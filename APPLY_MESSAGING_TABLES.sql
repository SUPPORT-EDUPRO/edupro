-- =====================================================
-- PARENT-TEACHER MESSAGING TABLES
-- Run this in Supabase SQL Editor to create messaging infrastructure
-- =====================================================
...
COMMENT ON TABLE message_threads IS 'Parent-teacher messaging threads with tenant isolation';
COMMENT ON TABLE message_participants IS 'Participants in each message thread';
COMMENT ON TABLE messages IS 'Individual messages within threads';
COMMENT ON FUNCTION get_or_create_parent_teacher_thread IS 'Get existing or create new parent-teacher thread for a student';

-- Verify tables were created
SELECT 'SUCCESS: message_threads table created' AS status 
WHERE EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'message_threads');

SELECT 'SUCCESS: message_participants table created' AS status 
WHERE EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'message_participants');
