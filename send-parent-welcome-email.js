...
/**
 * Send parent approval email for an existing parent account
 * This simulates the email that should have been sent during registration approval
 */
...
async function main() {
  const parentEmail = process.argv[2] || 'dipsroboticsgm@gmail.com';
  ...
  console.log(`📧 Preparing to send parent approval email to: ${parentEmail}\n`);
  ...
  if (resetError) {
    ...
    console.log('\n💡 Note: This might be normal if using anon key. Proceeding with generic link...');
  }

  // Get parent and student info from database
  const guardianName = 'King Makunyane';
  const studentName = 'Child ONE';
  const schoolName = 'Young Eagles Preschool';
  
  const resetPasswordUrl = `https://lvvvjywrmpcqrpvuptdi.supabase.co/auth/v1/recover?token=PASSWORD_RESET_TOKEN`;
  ...
  if (emailError) {
    ...
    // Log the email HTML for testing
    console.log('\n📄 Email HTML Preview (first 500 chars):');
    console.log(emailHtml.substring(0, 500) + '...\n');
    
    console.log('💡 Email may be in TEST MODE. Check Supabase logs for details.');
    process.exit(1);
  }
  ...
  console.log('\n📬 Parent should receive the email at:', parentEmail);
  console.log('🔐 They can use the link to set their password and log in');
}
